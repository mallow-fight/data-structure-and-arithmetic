function SecretPhone(phone: string) {
	return phone.replace(/(\d{3})\d{4}(\d{4})/, "$1****$2")
}

const secretPhone = SecretPhone("17312345678");

console.log(secretPhone);

function getVarsType(vars?: any) {
...
function onlyNumber(input?: string) {
	return /^[\d]+$/.test(input || "");
}
...
function onlyNumberOrLetter(input?: string) {
